...
func (t StreamingToken) String() string {
	posStr := fmt.Sprintf(
		...
		t.PresenceDataPosition, t.DeviceListPosition,
	)
	return posStr
}
...
func (t *StreamingToken) IsEmpty() bool {
	return t == nil || t.PDUPosition+t.TypingPosition+t.ReceiptPosition+t.SendToDevicePosition+t.InvitePosition+t.AccountDataPosition+t.PresenceDataPosition+t.DeviceListPosition == 0
}
...
func NewStreamTokenFromString(tok string) (token StreamingToken, err error) {
	...
	token = StreamingToken{
		...
		DeviceListPosition:   positions[7],
	}
	return token, nil
}
